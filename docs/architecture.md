...
3. Product and review records load from sample fixtures or processed public data.
4. Review NLP extracts aspects and sentiment labels from review text.
5. Evidence index retrieves review snippets for a product and preference query.
6. Recommendation models score products with popularity, rating, content, collaborative, and Two-Tower signals.
7. Hybrid rank combines model scores and evidence availability into `Recommendation` objects.
8. Offline evaluation measures the ranked product IDs against relevant products using precision, recall, and NDCG.
9. FastAPI returns recommendations and deterministic report text through typed endpoints.
10. Next.js renders the user profile, report summary, product cards, evidence snippets, model signals, and comparison rows.
